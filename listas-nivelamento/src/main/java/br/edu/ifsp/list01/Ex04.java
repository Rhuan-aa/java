...
public class Ex04 {
    ...
    int compute(int n, int c, int m) {
        int comprados, embalagens;

        if (m <= 0 || n <= 0 || c <= 0) return -1;

        comprados = n / c;
        embalagens = comprados;

        while (embalagens >= m){
            int novasEmbalagens = embalagens / m;
            comprados += novasEmbalagens;
            embalagens = (embalagens%m) + novasEmbalagens;
        }
